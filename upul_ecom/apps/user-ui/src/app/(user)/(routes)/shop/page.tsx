...
const Breadcrumbs = ({ 
  ...
  <nav className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-gray-400 mb-6 mt-5 md:mt-1 font-outfit">
    <Link href="/" className="hover:text-black transition-colors">Home</Link>
    <span>/</span>
    <Link 
      href="/shop" 
      className={`${!category && !search && !isNewArrival && !hasDiscount ? 'text-black font-bold' : 'hover:text-black'}`}
    >
      Shop
    </Link>
    
    {isNewArrival && (
      <><span>/</span><span className="text-black font-bold">New Arrivals</span></>
    )}
    {hasDiscount && (
      <><span>/</span><span className="text-black font-bold">Sale</span></>
    )}
    {category && !isNewArrival && !hasDiscount && (
      <><span>/</span><span className="text-black font-bold">{category.split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(' ')}</span></>
    )}
    {search && (
      <><span>/</span><span className="text-black font-bold">Search</span></>
    )}
  </nav>
);
...
function ShopContent() {
  ...
  const getPageTitle = () => {
    ...
    if (hasDiscount) return "Sale Collection"; 
    if (categorySlug) return formatCategoryName(categorySlug);
  ...
  const pageDescription = searchTerm 
    ...
    : hasDiscount 
    ? "Shop our sale collection with special discounts"
    : categorySlug
    ? `Shop our ${formatCategoryName(categorySlug)} collection`
  ...
  // ✅ 2. DYNAMIC LIMIT (Optimized for full rows)
  ...
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="animate-pulse font-outfit uppercase tracking-widest text-[10px] text-gray-400">
          Loading Collection...
        </div>
      </div>
    }>
