...
const SizeChartModal = ({
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-md transition-all duration-300"
      onClick={onClose}
    >
      {/* Close Button */}
      <button
      ...
      {/* Image Container */}
      <div
        className="relative flex items-center justify-center max-w-[90vw] max-h-[85vh] animate-in zoom-in-95 duration-300"
...
export default function ProductPage() {
  ...
  const handleAddToCart = async () => {
    ...
    if (quantity > remainingStock) {
      toast.error(`Limit reached for this item.`);
      return;
    }
  ...
  const handleBuyNow = async () => {
    ...
    const existingItemIndex = verificationItems.findIndex(
      (i) => i.sku === newItem.sku,
    );
    if (existingItemIndex > -1) {
      verificationItems[existingItemIndex] = {
        ...verificationItems[existingItemIndex],
        quantity:
          verificationItems[existingItemIndex].quantity + newItem.quantity,
      };
    } else {
    ...
    try {
      const { data } = await axiosInstance.post("/api/cart/verify", {
        ...
        <div className="space-y-8 font-outfit">
          <div className="space-y-2 border-b border-gray-100 pb-6">
            <div className="flex justify-between items-start">
              <h1 className="text-3xl md:text-4xl font-outfit font-bold text-gray-900 leading-tight">
                {product.name}
              </h1>
              <button
            ...
            <div className="pt-6 space-y-2 text-xs text-gray-500">
              ...
              {hasVariants && !selectedSize ? (
                <p className="text-blue-600 flex items-center gap-1 font-medium mt-2">
                  <AlertCircle size={14} /> Select a size to see stock
                </p>
              ) : remainingStock > 0 ? (
                ...
                <p className="text-red-500 flex items-center gap-1 font-medium mt-2">
          ...
          <div className="space-y-6">
            {hasVariants && (
              <div>
                <div className="flex justify-between items-center mb-3">
                  ...
                  {/* Text Link for Size Chart (Moved here) */}
                  {product.sizeChartUrl && (
                    <button
                      onClick={() => setSizeChartModalOpen(true)}
                      className="text-sm font-medium text-gray-500 underline hover:text-black transition-colors"
                    >
                      Size Chart
                    </button>
                  )}
            ...
            {/* Fallback for items with no variants but still have a size chart */}
            {!hasVariants && product.sizeChartUrl && (
              <div>
                <button
                  onClick={() => setSizeChartModalOpen(true)}
                  className="text-sm font-medium text-gray-500 underline hover:text-black transition-colors"
          ...
          <div className="flex flex-col gap-4 pt-4">
            ...
            <div className="flex items-center gap-3 h-14">
              <div
                className={`flex items-center border border-gray-200 rounded-lg h-full ${remainingStock === 0 && (!hasVariants || selectedSize) ? "opacity-50 pointer-events-none" : ""}`}
              >
                <button
                  onClick={handleDecreaseQty}
                  disabled={quantity <= 1}
                  className="p-4 hover:bg-gray-50 disabled:opacity-30 h-full flex items-center"
                >
                  <Minus size={16} />
                </button>
          ...
          <div className="pt-8">
            ...
            <AccordionItem title="Returns & Exchanges" icon={RotateCcw}>
              ...
              <ul className="list-disc pl-5 mt-2 space-y-1 text-gray-500">
                <li>
                  Exchanges within <b>14 days</b> of delivery.
                </li>
