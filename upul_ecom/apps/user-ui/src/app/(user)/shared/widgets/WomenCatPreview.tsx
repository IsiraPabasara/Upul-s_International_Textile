...
const WomenCategoryPreview = () => {
  const scrollRef = useRef<HTMLDivElement>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['women-preview'],
    queryFn: async () => {
      // Your fetch logic here
      const res = await axiosInstance.get('/api/products/shop?category=women&limit=8', { isPublic: true });
      return res.data;
    },
    staleTime: 1000 * 60 * 5,
  });
...
export default WomenCategoryPreview